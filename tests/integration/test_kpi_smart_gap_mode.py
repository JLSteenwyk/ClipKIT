...
class TestKPISmartGapsMode(object):
    ...
    def test_simple_long_description(self):
        """
        usage: clipkit simple_long_description.fa -m kpi-smart-gap
        """
        input_file = f"{here.parent}/samples/simple_long_description.fa"
        output_file = "output/simple_long_description.fa_kpi_smart_gaps"
        kwargs = dict(
            input_file=input_file,
            output_file=output_file,
            input_file_format='fasta',
        ...
        with open(
            f"{here.parent}/expected/simple_long_description.fa_kpi_smart_gaps", "r"
        ) as expected:
            expected_content = expected.read()
